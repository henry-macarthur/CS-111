...
int main(int argc, char **argv)
{
    ...
    {
        raise_err();
    } //connected via tcp, now time for TLS
	...
	if(context == NULL)
	{
		fprintf(stderr, "error with tls ctx \n");
		exit(1);
	}

	cur_ssl = SSL_new(context); 
	SSL_set_fd(cur_ssl, socketfd);
	if(SSL_connect(cur_ssl) == -1)
	{
		fprintf(stderr, "error setting up tls \n");
		exit(1);
	...
	if(SSL_write(cur_ssl, id_buffer, 13) < 0)
	{
		raise_err();
	}
	...
	while (1)
	{
		...
		else
		{
			...
			if ((polls[0].revents & POLLIN) == POLLIN)
			{
				...
				int status = SSL_read(cur_ssl, &buffer, 256); //get the current input from standard in
				if (status < 0)
				{
			...
			if (take_reading && (is_running == 1))
			{
				...
				if(current_time >= future_time)
					SSL_write(cur_ssl, temp_buff, strlen(temp_buff));
				//printf("%s:%s:%s %.1f\n", hour, minute, second, get_temp(value));
